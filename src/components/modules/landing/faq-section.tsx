...
const FAQAccordionItem = ({ faq }: { faq: FAQItem }) => {
  return (
    <details className="mb-4 rounded-lg bg-white p-4 group shadow-md">
      <summary className="flex items-center justify-between text-base md:text-xl lg:text-2xl font-medium text-black cursor-pointer">
        <span className="w-3/4">{faq.question}</span>
        <span className="flex items-center justify-center w-9 h-9 bg-purple-200 rounded-full">
          <span className="text-purple-500 transition-transform duration-200 group-open:rotate-180">
            ▲
          </span>
        </span>
      </summary>
      <p className="mt-2 text-black text-sm md:text-base lg:text-lg">{faq.answer}</p>
    </details>
  );
};
