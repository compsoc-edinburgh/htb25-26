...
export default function SigninDrawer({
  ...
  const onSubmit = async (values: SignInValues) => {
    ...
    try {
      ...
      if (e?.errors) {
        setErrors(
          e.errors.map((err: any) => err.message || err.longMessage || "An error occurred")
        );
      } else {
      ...
      <DialogContent className="flex flex-col items-center justify-center px-12 pb-32">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="w-full">
          <div className="flex flex-col items-center justify-center gap-5 my-10">
            <svg
              className="h-20 w-20 2xl:h-40 2xl:w-40"
              width="58"
              height="58"
              viewBox="0 0 178 156"
              fill="none"
            ...
            <h1 className="font-whyte text-4xl font-bold">Welcome back!</h1>
          </div>
          <div className="flex flex-col gap-6 w-full">
            <div className="grid gap-3">
              ...
              <Input
                ...
                data-error={formErrors.email ? "true" : undefined}
            ...
            <div className="grid gap-3">
              ...
              <Input
                id="password"
                type="password"
                data-error={formErrors.password ? "true" : undefined}
            ...
            {errors.length > 0 && (
              <ul className="my-3 box-border max-w-sm">
                {errors.map((error, index) => (
                  <li key={index} className="list-inside list-disc text-sm text-red-600">
                    {error}
                  </li>
                ))}
              </ul>
            ...
            <div className="relative text-center text-sm after:absolute after:inset-0 after:top-1/2 after:z-0 after:flex after:items-center after:border-t after:border-border ">
              <span className="relative z-10 bg-white px-2 text-zinc-600 text-xs">
            ...
            <p className="w-full text-center text-zinc-500 text-xs">
