...
export default function AuthDrawer({
  ...
  const onSignUpSubmit = async (values: AccountValues) => {
  ...
  const onForgotSubmit = async (values: EmailValues) => {
    if (!signInLoaded || !signIn) return;
    await signIn.create({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="scrollbar-hide flex flex-col items-center justify-center overflow-hidden px-12 pb-32">
        ...
        <div className="my-10 flex w-full flex-col items-center justify-center gap-5">
          ...
          <h1 className="text-center font-whyte text-4xl font-bold">{title}</h1>
        </div>
